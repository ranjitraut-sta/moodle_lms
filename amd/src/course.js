(() => {
  ...
  const amdcoursesidebarOpenBtn = document.getElementById(
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    if (activeLesson) {
      // 1. Target node ma keyboard focus pathaune
      activeLesson.focus();

      // 2. Jodi navigation list center text range bahira chha bhane explicit scroll garne
  ...
  if (nextBtn)
    nextBtn.addEventListener("click", () =>
      alert("Next lesson clicked (static demo)"),
    );

  const prevBtn = document.getElementById("prevLessonBtn");
  if (prevBtn)
    prevBtn.addEventListener("click", () =>
      alert("Previous lesson clicked (static demo)"),
    );
...
document
  .querySelector(".amd-course-video-fullscreen-btn")
