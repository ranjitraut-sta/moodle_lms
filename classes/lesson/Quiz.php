...
class Quiz implements LessonModuleInterface
{
    ...
    private function get_ddwtos_data($q): array
    {
        ...
        // Fetching from question_ddwtos using 'id' for ordering instead of 'no'
        ...
        $index = 1; // We use a manual index to map to [[1]], [[2]], etc.
        foreach ($choices as $choice) {
            $options[] = [
                'no' => $index,
                'groupno' => $choice->draggroup,
                'text' => format_string($choice->answer), // Moodle 4.4 uses 'answer' column
                'infinite' => !empty($choice->infinite),
            ];
            $index++;
        }

        // Match the [[1]], [[2]] placeholders
        preg_match_all('/\[\[(\d+)\]\]/', $text, $matches, PREG_SET_ORDER);
        ...
        foreach ($matches as $match) {
            ...
            $dropzone = '
            <span class="amd-ddwtos-inline-drop" data-no="' . $blankNo . '">
                <input type="text"
                       readonly
                       class="form-control d-inline-block"
                       style="width:120px; border:2px dashed #0d6efd; margin: 0 5px;">
            </span>
        ';
